...
def main():
    ...
    parser.add_argument("--lr", type=float, default=0.4e-3) # not 1e-3
    ...
    parser.add_argument("--checkpoint_dir", type=str, default="checkpoints/hybrid_scatt_unet")
    parser.add_argument("--log_dir", type=str, default="runs/hybrid_scatt_unet")
    ...
    model = HybridAttentionUNet(n_channels=1, n_classes=1, audio_dim=cond_dim,
                                        base_channels=args.base_channels,
                                        embed_dim=128, num_heads=4).to(device)
    
    #____________________________________________________________________________________
    criterion = CompositeSSL(lambda_l1=1.0, lambda_ssim=0.6, lambda_temporal=0.05, lambda_tv=0.001).to(device)
    # criterion = nn.L1Loss()
    ...
    print("Training Conditional U-Net with Self/Cross-Attention Fusion...")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if val_loss < best_val_loss:
            ...
            checkpoint_path = os.path.join(args.checkpoint_dir, f"hybrid_{epoch}.pth")
